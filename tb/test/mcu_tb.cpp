...
protected:
    ...
    void initializeInputs() override {
        ...
        // Initialize unpacked arrays (addresses and data)
    ...
    void tick() {
        ...
        if (bvalid_next_cycle) {
            ...
            top->m_axi_bvalid = 1;
            // Only clear bvalid_next_cycle if we see the handshake complete
            if (top->m_axi_bready) {
                printf("[TB] AXI Slave: BVALID/BREADY handshake completed\n");
    ...
    void applyRequests(const std::vector<MemoryRequest>& requests) {
        // FIXED: Clear all bits first, then set specific ones
        top->consumer_read_valid = 0;
        top->consumer_write_valid = 0;
    ...
    // FIXED: Helper function to check if a specific bit is set in packed array
    bool isReadReady(int thread_idx) {
        return (top->consumer_read_ready >> thread_idx) & 1;
    }
...
TEST_F(MCUTestbench, ScalarRead) {
    ...
    // FIXED: Use helper function to check packed array bit
    EXPECT_TRUE(isReadReady(SCALAR_LSU_IDX));
...
TEST_F(MCUTestbench, UncoalescedDivergentRead) {
    ...
    runTransaction(requests);
    // FIXED: Use helper function to check packed array bits
    EXPECT_TRUE(isReadReady(2));
    EXPECT_EQ(top->consumer_read_data[2], 100);
    EXPECT_TRUE(isReadReady(7));
    EXPECT_EQ(top->consumer_read_data[7], 250);
    EXPECT_TRUE(isReadReady(11));
